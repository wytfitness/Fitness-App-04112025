...
export const api = {
  ...
  saveFavoriteWorkout: (name, plan) =>
   ...
   async getWeeklySummary() {
      // last 7 days (client-side), but server also defaults if omitted
      const end = new Date();
      ...
      const res = await authedFetch(`/user-api?${qs.toString()}`);
      if (!res.ok) throw new Error(`weekly-summary ${res.status}`);
      return res.json(); // -> { range:{start,end}, workouts, calories, active_min }
    }, 
};
