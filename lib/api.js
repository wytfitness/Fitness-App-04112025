...
// GET /functions/v1/nutrition-lookup?ean=...
async function lookupEAN(ean) {
  const url = `${process.env.EXPO_PUBLIC_SUPABASE_URL}/functions/v1/nutrition-lookup?ean=${encodeURIComponent(ean)}`;
  const headers = {
    apikey: process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY,
  };
  // If you already attach the auth token globally, you can drop this:
