...
import { api, authedFetch } from "../../../lib/api"; // ← add authedFetch
...
function getRangeBounds(range) {
  ...
  const end = range === "Today" ? new Date(start.getTime() + 24 * 60 * 60 * 1000) : now;
  return { startMs: Date.parse(start.toISOString()), endMs: Date.parse(end.toISOString()) };
}
...
export default function History() {
  ...
  const [mealsToday, setMealsToday] = useState([]); // reuse for any range
  ...
  const loadWW = useCallback(async () => {
    ...
    try {
      ...
      const arrW = wWeights.weights ?? [];
      const arrWo = wWorkouts.workouts ?? [];
      setWeights(arrW);
      setWorkouts(arrWo);
    } catch (e) {
      setErr(String(e));
  ...
  const loadMeals = useCallback(async () => {
    ...
    try {
      ...
      if (range === "Today") {
        ...
        const meals = j?.meals ?? [];
        for (const m of meals) {
          const dateStr = new Date(m.eaten_at).toLocaleDateString();
          const label = mapType[m.meal_type ?? "snack"] ?? "Snack";
          for (const it of m.meal_items ?? []) {
            items.push({
              id: it.id,
              title: it.food_name,
              sub: `${dateStr} • ${label}${it.calories != null ? ` • ${Math.round(it.calories)} cal` : ""}`,
            });
          }
        }
      }
  ...
  useEffect(() => { loadMeals(); }, [loadMeals]); // ← reload when range changes
  useFocusEffect(useCallback(() => { loadWW(); loadMeals(); }, [loadWW, loadMeals]));

  // filter by range (using bounds)
  ...
  let data = [];
  if (tab === "Meals") {
    data = mealsToday; // ← show for all ranges
  } else if (tab === "Workouts") {
    data = filteredWorkouts
      .sort(
        (a, b) =>
          new Date(b.ended_at ?? b.started_at) - new Date(a.ended_at ?? a.started_at)
      )
      .map((w) => {
